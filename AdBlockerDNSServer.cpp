...
/* ESP32_AdBlocker */
      // Get domain IP from external DNS or set IP to 0.0.0.0 if domain in blocklist or not resolvable
      IPAddress gotIP = checkBlocklist(getDomainNameWithoutWwwPrefix(static_cast<const unsigned char *>(dnsQuestion.QName), dnsQuestion.QNameLength).c_str());
      for (int i=0; i<4; i++) _resolvedIP[i] = gotIP[i];
/* ESP32_AdBlocker */
